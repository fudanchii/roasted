...
use crate::{account::{Account, AccountStore}, parser::LedgerParser};
...
impl Transaction {
    pub fn parse(account_store: &AccountStore, date: NaiveDate, header: &str, txn: &str) -> Transaction {
        let mut trx_header = LedgerParser::parse(Rule::trx_header, header).unwrap_or_else(|e| panic!("{}", e));
        ...
        let txn_pairs = LedgerParser::parse(Rule::trx_list, txn).unwrap_or_else(|e| panic!("{}", e));
